...
def n2one(path_n_one,path_w_one):
    sga2deg = list()
    for line in open(path_n_one, 'r'):
        values = line.strip().split('\t')
        sga,deg = values[1],values[2]
        sga2deg.append(('leadTo',sga,deg,'1.0000'))
...
if __name__ == '__main__':
    ...
    path_w_one = dest+'/pathway_origin/pathway1.graph'

    n2one(path_n_one,path_w_one)
